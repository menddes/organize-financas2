...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const transformScheduledTransaction = (dbScheduledTransaction: any): ScheduledTransaction => {
    const categoryName = state.categories.find(c => c.id === dbScheduledTransaction.category_id)?.name || 'Unknown';
    return {
  ...
  useEffect(() => {
    console.log('AppContext: Filtering transactions...', {
    ...
    console.log('AppContext: Filtered transactions:', {
  ...
  const loadUserData = async (user: any) => {
    ...
    try {
      ...
      const [transactionsRes, categoriesRes, goalsRes, scheduledRes] = await Promise.all([
        ...
        supabase.from('poupeja_scheduled_transactions').select('*').eq('user_id', user.id)
      ]);
