...
import { Save, Upload, Loader2, Image, Globe } from 'lucide-react';
      ...
      try {
        ...
        if (data?.success && data?.data) {
          setFormData({
            ...
            faviconUrl: data.data.FAVICON_URL || '',
            companyName: data.data.COMPANY_NAME || '',
            logoAltText: data.data.LOGO_ALT_TEXT || '',
          });
        }
      } catch (err) {
  ...
  const handleSave = async () => {
    try {
      ...
      if (data?.success) {
        ...
        // Sinalizar atualização para o BrandingContext
        localStorage.setItem('branding-refresh', 'true');

        toast({
          title: "Configurações de branding salvas!",
          description: "Logo e favicon foram atualizados com sucesso.",
        });
      }
    } catch (error: any) {
