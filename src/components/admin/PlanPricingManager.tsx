...
const PlanPricingManager: React.FC = () => {
    ...
    const loadPricingConfig = async () => {
      try {
        ...
        if (data?.success && data?.data) {
          setFormData({
            planPriceMonthly: data.data.PLAN_PRICE_MONTHLY || '',
            planPriceAnnual: data.data.PLAN_PRICE_ANNUAL || '',
          });
        }
      } catch (err) {
  ...
  const calculateDiscount = () => {
    const monthly = parseFloat(formData.planPriceMonthly.replace(',', '.'));
    const annual = parseFloat(formData.planPriceAnnual.replace(',', '.'));
        ...
        {formData.planPriceMonthly && formData.planPriceAnnual && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            ...
            <div className="text-blue-700 text-sm space-y-1">
              <p>Valor mensal × 12: R$ {(parseFloat(formData.planPriceMonthly.replace(',', '.')) * 12).toFixed(2).replace('.', ',')}</p>
