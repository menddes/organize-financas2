...
const UpcomingPayments: React.FC<UpcomingPaymentsProps> = ({ 
  ...
  const upcomingPayments = scheduledTransactions
    .filter(transaction => {
      const transactionDate = new Date(transaction.scheduledDate);
        ...
        <div className={isMobile ? "space-y-2" : "space-y-3"}>
          {upcomingPayments.map((transaction) => {
            ...
            return (
              <div
                key={transaction.id}
                className={`flex items-center justify-between border rounded-lg hover:bg-gray-50 cursor-pointer transition-colors ${
                  isMobile ? 'p-2' : 'p-3'
                }`}
